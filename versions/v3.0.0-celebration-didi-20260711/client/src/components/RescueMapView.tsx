...
function buildNearbyFake(
  ...
  const fakeShelters = [
    { name: '同城流浪猫中转站', address: '当前位置附近（1km内模糊）', phone: '微信：shelter_01' },
    { name: '爱心救助驿站', address: '当前位置附近（1km内模糊）', phone: '微信：shelter_02' },
    { name: '社区猫咪之家', address: '当前位置附近（1km内模糊）', phone: '微信：shelter_03' },
  ];
  ...
  fakeForumTitles.forEach((f, idx) => {
    ...
    items.push({
      ...
      address: linked?.address || `当前位置附近（1km内模糊定位）`,
  ...
  const allItems = useMemo(() => {
    ...
    markers?.hospitals.forEach((h) => {
      fromApi.push({
        ...
        friendly_note: h.friendly_note || (
          h.discount_note
            ? `流浪猫友好：${h.discount_note}。送医时可说明来自「捡到猫了」。`
            : '接受流浪动物送医，建议电话确认救助通道与优惠。'
        ),
        discounts: h.prices?.map((p) => ({
          item: p.item,
          price: p.price,
          unit: p.unit,
          free: p.free,
      ...
      <div className="map-nearest-panel">
        <div className="map-nearest-head">
          <span className="font-bold text-sm text-[var(--ink-900)]">
            {tab === 'hospital'
              ? `附近友好医院（近→远 · ${nearestThree.length}）`
              : `最近 ${nearestThree.length} 个`}
            {tab !== 'hospital' && (tab === 'all' ? '地点' : TABS.find((t) => t.key === tab)?.label)}
          </span>
          <span className="text-[10px] text-[var(--ink-muted)]">
            {tab === 'hospital' ? '含折扣说明 · 可导航定位' : '点击展开 · 求助进流浪发现'}
          </span>
        </div>
                  ...
                  <div className="map-list-detail">
                    {it.category === 'forum' && (
                      <>
                        ...
                        <p className="text-[11px] text-amber-700 mt-2">📍 {it.address}（模糊定位）</p>
                        {it.contact && (
