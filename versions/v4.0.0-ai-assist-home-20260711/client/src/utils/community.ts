...
export function amapNavUrl(lng: number, lat: number, name?: string) {
  ...
  return `https://uri.amap.com/navigation?to=${encodeURIComponent(dest)}&mode=car&src=jiandaomao`;
}

/** 打开高德地图标记点（查看位置） */
export function amapMarkerUrl(lng: number, lat: number, name?: string) {
  const n = encodeURIComponent(name || '目的地');
  return `https://uri.amap.com/marker?position=${lng},${lat}&name=${n}&src=jiandaomao`;
}

/** 用系统地图打开坐标（备选） */
export function geoUri(lat: number, lng: number, name?: string) {
