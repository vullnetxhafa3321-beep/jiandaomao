...
默认走免费 [Pollinations](https://text.pollinations.ai/openai)（无需密钥）。可选在 Vercel / `server/.env` 配置更稳的上游（**切勿提交密钥**）：

- `AI_BASE_URL`（如 `https://api.groq.com/openai/v1`）
- `AI_API_KEY`
- `AI_MODEL`（如 `llama-3.3-70b-versatile`）

前端只请求同源 `/api/ai/chat/completions`，密钥留在服务端。
