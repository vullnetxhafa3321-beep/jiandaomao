...
import { registerAiRoutes } from './ai-chat.js';
...
/** 已停用开放代理：禁止经查询串转发 client_id/secret */
app.get('/api/baidu/token', (_req, res) => {
  res.status(410).json({
    error: '开放 token 代理已停用。请仅使用 POST /api/baidu/recognize（密钥只在服务端）',
  });
});

app.post('/api/baidu/animal', (_req, res) => {
  res.status(410).json({
    error: '开放 animal 代理已停用。请仅使用 POST /api/baidu/recognize',
  });
});

/** 服务端持钥识别（上传文件 / base64 / 已有图片 URL） */
app.post('/api/baidu/recognize', upload.single('image'), async (req, res) => {
  try {
    if (!baiduConfigured()) {
      return res.status(503).json({
        error: '百度 AI 未配置。请在服务端设置 BAIDU_API_KEY 与 BAIDU_SECRET_KEY（勿写入前端 / 仓库）',
        configured: false,
      });
    }
    ...
    const data = await recognizeAnimalBuffer(buffer);
