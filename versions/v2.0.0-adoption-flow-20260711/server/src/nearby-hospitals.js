...
const BRANCH_SUFFIX = ['附近友好店', '同城合作店', '街猫通道店', '义诊点', '急诊友好店'];

/**
 * Build nearby friendly hospitals around user — never empty, at least 10.
...
export function buildNearbyFriendlyHospitals(lat, lng, limit = 12) {
  ...
  while (pool.length < target) {
    const t = TEMPLATES[i % Math.max(TEMPLATES.length, 1)] || {
      ...
      hours: '09:00–21:00',
      isPartner: true,
      discount_note: '流浪猫绝育优惠 · 基础体检免费（演示）',
      friendly_note: FRIENDLY_TEMPLATES[i % FRIENDLY_TEMPLATES.length],
  ...
  return pool
    .map(({ t, i: idx, id, name }) => {
      ...
      return {
        ...
        discount_note: t.discount_note,
        partnerDiscount: t.discount_note,
        friendly_note: t.friendly_note,
        prices: t.prices,
        tags: t.tags,
...
export function fuzzyAddressLabel(address) {
  ...
  if (!raw) return '发现地附近（1km内模糊定位）';
  if (raw.includes('模糊定位')) return raw;
  const area = raw
    .replace(/（[^）]*）/g, '')
    .replace(/\([^)]*\)/g, '')
    .replace(/\d+号.*$/, '')
