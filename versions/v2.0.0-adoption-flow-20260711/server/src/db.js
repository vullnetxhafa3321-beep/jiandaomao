...
/** Local: server/data ; Vercel: /tmp (ephemeral but seeded on cold start) */
const dataDir = isServerless ? '/tmp/jiandaomao-data' : path.join(__dirname, '..', 'data');
...
export function initDb() {
  ...
  try { db.exec('ALTER TABLE forum_posts ADD COLUMN contact TEXT'); } catch { /* exists */ }
  try { db.exec('ALTER TABLE forum_posts ADD COLUMN rescuer_name TEXT'); } catch { /* exists */ }
